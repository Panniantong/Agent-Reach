...
| 🔍 **全网搜索（Exa）** | — | 全网语义搜索 | 自动配置（MCP 接入，免费无需 Key） |
| 🤖 **知识图谱 & 全网搜索** | — | 全网搜索 + DQL 结构化检索公司/人物/文章 | 告诉 Agent「帮我配 Diffbot」（[diffbot.com](https://www.diffbot.com)；免费 Token，`db dql init` 缓存本体） |
...
├── diffbot_search.py → Diffbot Web Search via db CLI（diffbot-python）
├── diffbot_kg.py   → Diffbot 知识图谱 DQL via db dql（diffbot-python）
...
| 查新闻与公司 | [Diffbot 知识图谱（DQL）](https://github.com/diffbot/diffbot-python) | — | 结构化检索公司/人物/新闻文章；diffbot-python CLI；免费 API Token |
