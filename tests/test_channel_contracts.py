...
def test_youtube_warns_when_node_only_and_no_config(monkeypatch, tmp_path):
    ...
    # Point to a non-existent config file. Patch the resolver the channel
    # actually calls — on Windows the path comes from %APPDATA% (not
    # os.path.expanduser), so patching expanduser alone leaks the real config.
    monkeypatch.setattr(
        "agent_reach.channels.youtube.get_ytdlp_config_path",
        lambda: tmp_path / "yt-dlp" / "config",
    )
