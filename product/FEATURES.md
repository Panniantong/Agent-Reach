...
Status: **planned** (not started in code) · **blocked-on-Pepe** (needs a pick before code) · **done** (shipped in repo). Specs in this directory are Phase 0 deliverable; they are not "done" as product features.
...
|----|------|-------|--------|----------|------|
| F1 | Contract truth | 1 | planned | mixed | [F01](features/F01-contract-truth.md) |
| F2 | Install/update hygiene | 1–2 | planned | mixed | [F02](features/F02-install-hygiene.md) |
| F3 | Doctor honesty | 1 | planned | JSON field blocked | [F03](features/F03-doctor-honesty.md) |
...
| F8 | Channel productization | 1+4 | planned | `--channels=all` | [F08](features/F08-channel-productization.md) |
| F9 | MCP | 1+keep | planned | no new tools | [F09](features/F09-mcp.md) |
| F10 | Packaging/distribution | 5 | planned | 1.6 vs 2.0 | [F10](features/F10-packaging.md) |
| F11 | Tests/CI | 1–5 | planned | no | [F11](features/F11-tests-ci.md) |
| F12 | Telemetry/update | 1 | planned | nudge YES | [F12](features/F12-telemetry-update.md) |

## Subfeatures (all)

| ID | Name | Parent | Phase | Status | Approval needed? |
|----|------|--------|-------|--------|------------------|
| F1.1 | CLAUDE.md contract + count | F1 | 1 | planned | no |
| F1.2 | README commercial story + `product/` pointer | F1 | 1 | planned | no (pointer YES) |
| F1.3 | Skill dual-language cookie-paste line | F1 | 1 | planned | no |
| F1.4 | CONTRIBUTING registry truth | F1 | 1 | planned | no |
| F1.5 | Version three-place assert | F1 | 1 | planned | no |
| F1.6 | `config.json` → `config.yaml` in install docs | F1 | 1 | planned | no |
| F1.7 | pyproject `description` wording | F1 | 1 | planned | no (text only, no release) |
| F1.8 | Kill "entire internet" in package strings | F1 | 1 | planned | no (docstrings/help, not flags) |
| F1.9 | Translation README taglines | F1 | 1 | planned | no |
| F1.10 | Registry count test `== 15` | F1 | 1 | planned | no |
| F1.11 | Skill "zero-config" count honesty | F1 | 1 | planned | no |
| F2.1 | `constraints.txt` on default install path | F2 | 1 | planned | no |
| F2.2 | `--system` blast-radius docs | F2 | 1 | planned | no |
| F2.3 | Check-only default stays | F2 | 1 | planned (lock) | no |
| F2.4 | Documented one-liner is check-only | F2 | 1 | planned | no |
| F2.5 | Skill install paths documented | F2 | 1 | planned | no |
| F2.6 | `docs/update.md` constraints recipe | F2 | 1 | planned | no |
| F2.7 | `--system` CLI blast print | F2 | 2 | planned | yes if new flag |
| F2.8 | pipx/`-c` limitation documented | F2 | 1 | planned | no |
| F2.9 | Uninstall skill-dir list stays accurate | F2 | 1 | planned (docs) | no |
| F3.1 | Login-channel false-negatives, prose | F3 | 1 | planned | no |
| F3.2 | `doctor --json` `confidence` field | F3 | 1 | **blocked-on-Pepe** | yes (public API). Skip Phase 1 |
| F3.3 | JSON schema freeze (`status` enum) | F3 | 1 | planned (lock) | no |
| F3.4 | Document which channels can never be `ok` | F3 | 1 | planned | no |
...
| F8.1 | Commercial set first in README/skill | F8 | 1 docs / 4 gate | planned | no for docs |
...
| F9.2 | Advertised `[mcp]` extra does not exist | F9 | 1 docs / 5 extra | planned | yes to add extra |
...
| F11.1 | Docs/contract grep tests | F11 | 1 | planned | no |
| F11.2 | Doctor tests (prose, not new JSON field) | F11 | 1 | planned | no |
| F11.3 | Install/constraints tests | F11 | 1 | planned | no |
...
| F12.1 | Drop skill check-update nudge | F12 | 1 | planned | no (YES encoded) |
| F12.2 | Keep `check-update` as user-invoked | F12 | 1 | planned (keep) | no |
| F12.3 | `watch` GitHub GET stays user-scheduled | F12 | 1 | planned (keep) | no |
| F12.4 | `config.check_update_on_skill` | F12 | skip | **blocked-on-Pepe** | yes (config key). Do not add |

## Phase 1 execute set (next chat)

Do these. Do not do later phases.
