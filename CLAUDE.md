...
- `agent_reach/core.py` — `AgentReach` is doctor-only (`doctor` / `doctor_report`); not a read/search router
...
- Each channel is a single file in `channels/`, inherits from `Channel` in `base.py`
- Channel contract: must implement `can_handle(url)` and `check()` only (`read`/`search` are not required)
...
- Version in THREE places must match: `pyproject.toml`, `agent_reach/__init__.py`, and the assert in `tests/test_docs_contract.py`
