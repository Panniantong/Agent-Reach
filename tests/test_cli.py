...
class TestCLI:
    ...
    def test_install_opencli_channels_route_to_opencli_once(self, monkeypatch, capsys):
        calls = []
        ...
        cli._cmd_install(
            Namespace(
                ...
                safe=False,
                dry_run=False,
                channels="facebook,instagram,nowcoder,opencli",
            )
        )
    ...
    def test_install_server_dry_run_skips_opencli_only_channels(self, monkeypatch, capsys):
        ...
        cli._cmd_install(
            Namespace(
                ...
                safe=False,
                dry_run=True,
                channels="facebook,instagram,nowcoder,opencli,bilibili",
            )
        )

        out = capsys.readouterr().out
        assert "服务器环境跳过：facebook, instagram, nowcoder, opencli" in out
        assert "[dry-run] Would install optional channels: bilibili" in out
        assert "facebook, instagram, nowcoder, opencli, bilibili" not in out
